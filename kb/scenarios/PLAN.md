...
We start with `kb/scenarios/` and add scenario docs for common, code-centric Kirby tasks found in:
...
- Panel extensions (areas, fields, sections, dialogs, dropdowns, searches)
...
- **Version notes (K5/K6)** (optional; include when behavior differs)
...
Naming: default is `NN-title.md` for stable ordering; Panel extension scenarios may use
`panel-` prefixes to group related tasks.
...
## Panel extension scenarios (Cookbook)

These are Panel plugin scenarios derived from the Cookbook Panel recipes. Use
`panel-` prefixes and include K5/K6 notes when relevant. Cross-link to
`kirby://kb/panel/INDEX` reference docs.
