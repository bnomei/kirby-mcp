...
- `kb/glossary/` (terminology)
- `kb/scenarios/` (task playbooks)
...
- `kb/update-schema/<field>.md` (one field type per file, lowercase slug)
- Optional: `kb/content/INDEX.md` (alphabetical index + categories)
- Resource mapping (future): `kirby://field/{field}/update-schema` -> `kb/update-schema/<field>.md`
...
- For each match, add a short note or link pointing to `kirby://field/{field}/update-schema` (or the relevant field doc under `kb/update-schema/`).
