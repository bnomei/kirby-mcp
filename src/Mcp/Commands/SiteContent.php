...
final class SiteContent extends RuntimeCommand
{
    /**
     * @return array{
     *   description: string,
    ...
    {
        return [
            'description' => 'Reads the Kirby site content (structured output for MCP; prefer the `kirby_read_site_content` tool or `kirby://site/content` resource).',
            'args' => [
                'language' => [
                    'longPrefix' => 'language',
    ...
    public static function run(CLI $cli): void
    {
        ...
        try {
            ...
            $content = $site->content($lang)->toArray();
            $fieldSchemas = FieldSchemaHelper::fromFieldDefinitions($site->blueprint()->fields(), true);
            ...
            if ($presentComplex !== []) {
                ...
                $warningBlock = [
                    'message' => 'WARNING: This site contains ' . implode('/', $presentComplex)
            ...
            $beforeUpdateRead = ['kirby://blueprint/site/update-schema'];
            ...
            $payload = [
                ...
                'warning' => 'Do not edit Kirby content files directly unless explicitly asked. Prefer the Panel or a safe tool like kirby_update_site_content. For payload shapes, see kirby://field/{type}/update-schema.',
            ];
