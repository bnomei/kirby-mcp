...
- Shared-token HTTP auth is loopback/local-development only. Production or non-loopback HTTP exposure must use OAuth JWT validation with issuer, audience/resource, signature/JWKS, expiry, and scope checks; the Kirby route must reject shared-token requests unless PHP reports `REMOTE_ADDR` as loopback.
