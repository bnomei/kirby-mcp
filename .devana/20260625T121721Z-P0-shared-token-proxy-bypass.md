...
DEVANA-KEY: src/Mcp/KirbyMcpRoute.php:50 | shared-token-proxy-bypass
DEVANA-SUMMARY: fixed | P0 | high | Shared-token HTTP MCP trusts loopback REMOTE_ADDR only, so reverse-proxied public requests can reach the full MCP API with the shared bearer secret.